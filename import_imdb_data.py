...
with DBInterface("imdb") as db_interface:
    ...
    selected_movies_reviews["helpful_found"] = selected_movies_reviews.helpful.apply(
        lambda x: x[0] if not ',' in x[0] else 0).astype(int)
    element_texts = []
    ...
    for index, movie in tqdm(selected_movies.iterrows(), total=selected_movies.shape[0]):
        ...
        reviews = selected_movies_reviews[(selected_movies_reviews.movie == movie.primaryTitleWithYear) & selected_movies_reviews.rating.notna(
        )].sort_values(by='helpful_found', ascending=False).iloc[:REVIEW_SAMPLE_PER_ITEM]
