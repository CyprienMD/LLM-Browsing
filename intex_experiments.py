...
def get_target_ids():
    ...
    return pd.read_csv(f"rl/targets/{dataset}_{target_variant}.csv").id.to_list()


def get_validation_target_ids():
    dataset = configuration.learning_configurations["dataset"]
    target_variant = configuration.learning_configurations["target_variant"]
    ...
    if transfer_variant == "":
        ...
        return pd.read_csv(f"rl/targets/{dataset}_{target_variant}_{transfer_variant}.csv").id.to_list()
...
def target_query(test=False):
    ...
    if test and transfer_variant == "G2S":
        limit = 8
    if test and transfer_variant == "S2G":
        limit = 2

    item_offset_clause = (
        "offset {}".format(
            limit) if test and transfer_variant == "DIFF" else ""
    )
    element_offset_clause = (
        "order by type desc offset 100" if test and transfer_variant == "SIM" else ""
    )
    ...
    if dataset == "amazon":
        ...
        focus_values = (
            ["Computers"]
            if target_variant == "T1"
            else ["Camera &amp; Photo", "Camera & Photo"]
        )
    ...
    target_query = "select item_id from elements where item_id in (select element_id from attributes where name = '{}' and ({})) group by item_id order by sum(rating) desc {} limit {};".format(
        focus_attribute, value_condition, item_offset_clause, limit
    )
    target_element_ids = []
    ...
    target_element_ids_str = ",".join(str(value)
                                      for value in target_element_ids)

    return "select id, rating, type from elements where id in ({}) UNION select id, rating, type from elements where item_id in ({}) {}".format(
        target_element_ids_str, target_element_ids_str, element_offset_clause
    )
