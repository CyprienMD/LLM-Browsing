...
with DBInterface(dataset) as db_interface:
    ...
    if algorithm == "DQN":
        ...
        q_func = pfrl.nn.RecurrentSequential(
            torch.nn.Linear(observation_size, network_width),
            torch.nn.ReLU(),
        ...
        model = torch.nn.Sequential(
            torch.nn.Linear(observation_size, network_width), torch.nn.ReLU(),
        ...
        agent = pfrl.agents.A2C(model, optimizer, gamma,
    ...
    feature_activation_columns = ["none", "diverse_numerical", "diverse_review", "coverage_review",
